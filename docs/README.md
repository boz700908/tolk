...
### Wrappers
Wrappers around `Tolk.dll` have been added for some languages to make things easier. Each wrapper is placed in its own language-specific subdirectory within the build output:

* **.NET (C#/VB.NET)**: `TolkDotNet.dll` → `dist/<arch>-<config>/dotnet/`
* **Java**: `Tolk.jar` → `dist/<arch>-<config>/java/`
* **Python**: `Tolk.py` (with `.pyc`/`.pyo` bytecode) → `dist/<arch>-<config>/python/`
* **AutoIt**: `Tolk.au3` → `dist/<arch>-<config>/autoit/`
* **PureBasic**: `Tolk.pb` → `dist/<arch>-<config>/purebasic/`
