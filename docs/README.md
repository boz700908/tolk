...
* **.NET (C#/VB.NET)**: `TolkDotNet.dll` → `<arch>-<config>/dotnet/`
* **Java**: `Tolk.jar` → `<arch>-<config>/java/`
* **Python**: `Tolk.py` (with `.pyc`/`.pyo` bytecode) → `<arch>-<config>/python/`
* **AutoIt**: `Tolk.au3` → `<arch>-<config>/autoit/`
* **PureBasic**: `Tolk.pb` → `<arch>-<config>/purebasic/`
